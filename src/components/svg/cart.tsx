...
export default function CartSvg() {
    ...
    useEffect(() => {
        setNum(cartItems.reduce((acc, item) => acc + item.quantity, 0));
    }, [cartItems])

    return (
        <svg className="w-[30px] h-[30px] fill-none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <g>
            ...
            <text x="18" y="6" textAnchor="middle" dy=".3em" className="fill-white text-[9px]">{num}</text>
        </svg>
    )
}